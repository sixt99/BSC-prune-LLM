...
def plot_matrix_analysis(matrix, visualization_mode=None, show_gaussian = True, ignore_zeros = False):
    ...
    # Plot histogram
    if not ignore_zeros:
      axs[1].hist(matrix.flatten(), bins=200, density=True, alpha=0.7, range=(lim_a, lim_b))
    else:
      flat = matrix.flatten()
      axs[1].hist(flat[flat != 0], bins=200, density=True, alpha=0.7, range=(lim_a, lim_b))
...
def print_weight_matrices(model, visualization_mode=None, show_gaussian = True, ignore_zeros = False):
  for x in model.state_dict().keys():
    ...
    if len(matrix.shape) > 1:
      print(x)
      plot_matrix_analysis(matrix, visualization_mode, show_gaussian, ignore_zeros)